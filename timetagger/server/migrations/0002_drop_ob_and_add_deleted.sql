...
-- records ------------------------------------------------------------------
ALTER TABLE records ADD COLUMN IF NOT EXISTS mt bigint;
...
-- settings -----------------------------------------------------------------
ALTER TABLE settings ADD COLUMN IF NOT EXISTS mt bigint;
ALTER TABLE settings ADD COLUMN IF NOT EXISTS value jsonb;

UPDATE settings
SET mt = (_ob->>'mt')::bigint, value = _ob->'value'
WHERE _ob IS NOT NULL;

ALTER TABLE settings DROP COLUMN IF EXISTS _ob;

-- userinfo -----------------------------------------------------------------
ALTER TABLE userinfo ADD COLUMN IF NOT EXISTS mt bigint;
